...
for sample in samples:
    if not sample.Name.count("GENSIM"):
        continue

    os.mkdir( "%s/%s" % (workingdir , sample.Name) )
    copy( "SetupAndRunPremix.sh" , "./%s/%s" % (workingdir , sample.Name) )

    file = open("%s/%s/Submit.cmd" % (workingdir , sample.Name) , "w" )
    print >> file, "executable              = %s/%s/%s/SetupAndRunPremix.sh" % (os.getcwd() , workingdir , sample.Name)
