...
datanTuples = "%s/Hamb13/June06_Full2016_MassProd/withSelVariables/withMoreBtags/" %(LOCATION) #"/afs/cern.ch/work/a/ajafari/HAMB/CMSSW_8_0_26_patch1/src/Haamm/HaNaMiniAnalyzer/macro/out/" # 

#on hamed laptop :
import os
if os.uname()[1] == "cp3lap20" :
    nTuples = "/home/hbakhshi/Downloads/HambApproval/"
    datanTuples = "/home/hbakhshi/Downloads/HambApprovalData/"
####

#nTuples = nSystTuples = datanTuples
...
signalsamples.append (SampleType( "SignalVBF20" , kRed+2 , [ GetSample(VBF2080) ] , nTuples , True ))
signalsamples.append (SampleType( "SignalVBF40" , kRed+2 , [ GetSample(VBF4080) ] , nTuples , True ))
signalsamples.append (SampleType( "SignalVBF60" , kRed+2 , [ GetSample(VBF6080) ] , nTuples , True ))
nTotals = {}
myChi2Tot = "chi2Sum"            
...
for cut in cuts :
    ...
    chi2sum = cut.AddHist( "chi2Sum" , myChi2Tot, 25 , 0. , 50., False , Title="#chi^{2}_{H}+#chi^{2}_{a#rightarrowbb}", dirName="Jets")
    amuMass = cut.AddHist( "amuMass" , "aMuMass", 140 , 10 , 80 , Title="#mu#mu mass" , dirName="General")
    abmass  = cut.AddHist( "bbMass" , "abMass" , 90, 10. , 100., False , Title="m_{bb}" , dirName="General")
    ...
    chi2b=cut.AddHist( "chi2B" , "chi2B", 100 , 0. , 50., False , Title="#chi^{2}_{a#rightarrowbb}" , dirName="bb")
    linchib=cut.AddHist( "LinearChi2B" , "(abMass - aMuMass)/(%s)" %bbWidth, 80, -50. , 50., False , Title="(m_{bb}-m_{#mu#mu})/#sigma_{bb}" , dirName="bb")
    linchih=cut.AddHist( "LinearChi2H" , "(higgsMass - 125)/10.6", 100 , -20. , 100., False , Title="(m_{bb#mu#mu}-125)/#sigma_{h}" , dirName="MuMubb")
    chi2h=cut.AddHist( "chi2H" , "chi2H", 100 , 0. , 50., False , Title="#chi^{2}_{H#rightarrowbb#mu#mu}" , dirName="MuMubb" )
    cut.AddHist( "chi2SUM" , myChi2Tot, 50 , 0. , 50., False , Title="Sum #chi^{2}" , dirName="MuMubb" )
    cut.AddHist( "HiggsMDiff" , "abs(higgsMass - 125.0)", 20 , 0. , 100., False , Title="|mass_{#mu#mubb}-125.0|" , dirName="MuMubb" )
    cut.AddHist( chi2b , chi2h)
    cut.AddHist( linchib , linchih)
    cut.AddHist( amuMass , abmass )
